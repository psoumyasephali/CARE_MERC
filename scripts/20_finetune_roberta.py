"""Fine-tune RoBERTa-base on MELD train for emotion classification, then
extract [CLS] features for train/dev/test.

Outputs:
  features/{split}/text_finetuned.npy  (N, 768) float32
  features/{split}/consolidated_v2.npz  (text swapped, A/V/labels/etc. unchanged)
  checkpoints/roberta_meld_ft.pt
  results/roberta_finetune.json
"""
...
def main() -> None:
    ...
    ckpt_path = Path(args.ckpt_dir) / "roberta_meld_ft.pt"
    history = []
    best_dev_wf1 = -1.0
    best_epoch = -1
    bad_epochs = 0
    ...
    for split, loader_ in [("train", train_extract), ("dev", dev_extract), ("test", test_extract)]:
        ...
        out = Path(args.features_dir) / split / "text_finetuned.npy"
        np.save(out, f)
        print(f"  saved {out}  shape={f.shape}  dtype={f.dtype}", flush=True)
        feats[split] = f

    # Build consolidated_v2.npz for each split with text swapped
    print("[merge] building consolidated_v2.npz", flush=True)
    for split in ["train", "dev", "test"]:
        ...
        out = Path(args.features_dir) / split / "consolidated_v2.npz"
        np.savez(
            out,
    ...
    with open(Path(args.results_dir) / "roberta_finetune.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"[save] {Path(args.results_dir) / 'roberta_finetune.json'}", flush=True)
